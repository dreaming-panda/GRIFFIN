...
class XHFLM(TemplateLM):
    ...
    def _create_model(
        ...
        **kwargs,
        ...
        schedule_k = [0.5 for _ in range(self._model.config.num_hidden_layers)]
        self._model.config.mode = "gen"
        self._model.config.selection_method = "topk"
        self._model.config.check = check
        self._model.config.griffin = griffin
        self._model.config.kernel_size = kernel_size
        #self._model.eval()
        if griffin:
            self._model = get_llama_griffin(self._model, schedule_k)
        # self._model = XLlamaForCausalLM.from_pretrained(
        #         pretrained,
        #         torch_dtype=get_dtype(dtype),
        #         device_map=str(self.device),
        #         _attn_implementation="eager",
        #     )
        #self._model.set_sparse_attn(**kwargs)
        return None
